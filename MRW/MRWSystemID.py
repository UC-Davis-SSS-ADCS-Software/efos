...
def DataSave(DutyCycle):
    ...
    while (MRW_ON == True):
        ...
        data = str(DutyCycle.value), currentSpeed, start_time, end_time
...
def main():
    ...
    # Set duty cycle
    #absDC = np.linspace(0, 100, 11)
    #absDCRamped = np.concatenate([absDC, np.flip(absDC[:-1])])
    #allDC = np.concatenate([absDCRamped, -1 * absDCRamped[1:]])
    allDC = [10, 50, 80]

    arrSize = np.size(allDC)
    localstart = time.time()
    i = 0
    
    while (i < arrSize):
        DutyCycle.value = allDC[np.mod(i, arrSize)]
    ...
    # Plot result
    rpm, secondsElapsed = np.loadtxt(outfile, unpack=True, skiprows=1, delimiter=',', usecols=(0,1))
